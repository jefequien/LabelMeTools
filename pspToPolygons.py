...
def process(category):
    ...
    for f in files:
        ...
        if ext == ".png":
            ...
            data = createData(category_mask, prob_mask, max_prob)
            polygon_file = "{}/{}-polygons.json".format(polygons_path, fname)
            with open(polygon_file, 'w') as outfile:
                json.dump(data, outfile)

def createData(category_mask, prob_mask, max_prob):
...
if args.list:
    ...
    if len(args.process) == 1:
        categories = listOptions(args.process)
    else:
        categories = [args.process]
